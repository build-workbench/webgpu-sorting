...
describe('Validator', () => {
  ...
  describe('Property 8: isSorted Validator Correctness', () => {
    ...
    it('should return false for unsorted arrays with at least 2 elements', () => {
      fc.assert(
        fc.property(fc.array(fc.nat(1000000), { minLength: 2, maxLength: 1000 }), (arr) => {
  ...
  describe('Property 9: hasSameElements Validator Correctness', () => {
    it('should return true for arrays with same elements (permutations)', () => {
      fc.assert(
        fc.property(fc.array(fc.nat(1000000), { minLength: 0, maxLength: 500 }), (arr) => {
    ...
    it('should handle edge cases', () => {
      ...
      expect(
        Validator.hasSameElements(new Uint32Array([1, 2, 3]), new Uint32Array([3, 1, 2]))
      ).toBe(true);

      // Duplicates
      expect(
        Validator.hasSameElements(new Uint32Array([1, 1, 2]), new Uint32Array([1, 2, 1]))
      ).toBe(true);

      // Different duplicate counts
      expect(
        Validator.hasSameElements(new Uint32Array([1, 1, 2]), new Uint32Array([1, 2, 2]))
      ).toBe(false);
    });
  });
