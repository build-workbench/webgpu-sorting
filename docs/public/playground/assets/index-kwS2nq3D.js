(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class extends Error{constructor(e=`WebGPU is not supported in this browser`){super(e),this.name=`WebGPUNotSupportedError`}},t=class extends Error{constructor(e=`Failed to get GPU adapter`){super(e),this.name=`GPUAdapterError`}},n=class extends Error{constructor(e=`Failed to get GPU device`){super(e),this.name=`GPUDeviceError`}},r=class extends Error{constructor(e=`Failed to allocate GPU buffer`){super(e),this.name=`BufferAllocationError`}},i=class extends Error{constructor(e=`Failed to map GPU buffer`){super(e),this.name=`BufferMapError`}},a=class extends Error{constructor(e=`Failed to compile shader`){super(e),this.name=`ShaderCompilationError`}},o=class extends Error{constructor(e=`GPU operation timed out`){super(e),this.name=`GPUTimeoutError`}};function s(){return typeof navigator>`u`||!(`gpu`in navigator)?null:navigator.gpu}var c={isSupported(){return s()!==null},async requestAdapter(e){return await s()?.requestAdapter(e)??null}},l=class{adapter=null;device=null;initialized=!1;deviceLossCallbacks=new Set;limitsInfo=null;runtime;constructor(e=c){this.runtime=e}static isSupported(e=c){return e.isSupported()}onDeviceLoss(e){return this.deviceLossCallbacks.add(e),()=>{this.deviceLossCallbacks.delete(e)}}async recover(e){this.initialized=!1,this.device=null,this.adapter=null,this.limitsInfo=null,await this.initialize(e)}getLimitsInfo(){if(!this.limitsInfo)throw new n(`GPUContext not initialized. Call initialize() first.`);return this.limitsInfo}async initialize(r){if(this.initialized)return;if(!this.runtime.isSupported())throw new e;if(this.adapter=await this.runtime.requestAdapter({powerPreference:r?.powerPreference??`high-performance`}),!this.adapter)throw new t;let i=this.adapter.limits;this.limitsInfo={maxStorageBufferBindingSize:i.maxStorageBufferBindingSize,maxComputeInvocationsPerWorkgroup:i.maxComputeInvocationsPerWorkgroup,maxComputeWorkgroupSizeX:i.maxComputeWorkgroupSizeX,maxBufferSize:i.maxBufferSize};let a={maxStorageBufferBindingSize:Math.min(i.maxStorageBufferBindingSize,r?.requiredLimits?.maxStorageBufferBindingSize??i.maxStorageBufferBindingSize),maxBufferSize:Math.min(i.maxBufferSize,r?.requiredLimits?.maxBufferSize??i.maxBufferSize)};if(this.device=await this.adapter.requestDevice({requiredFeatures:[],requiredLimits:a}),!this.device)throw new n;this.device.lost.then(e=>{console.error(`GPU device lost:`,e.message),this.initialized=!1,this.device=null;for(let t of this.deviceLossCallbacks)try{t(e)}catch(e){console.error(`Error in device loss callback:`,e)}}),this.initialized=!0}getDevice(){if(!this.device)throw new n(`GPUContext not initialized. Call initialize() first.`);return this.device}getAdapter(){if(!this.adapter)throw new t(`GPUContext not initialized. Call initialize() first.`);return this.adapter}isInitialized(){return this.initialized}destroy(){this.deviceLossCallbacks.clear(),this.device&&=(this.device.destroy(),null),this.adapter=null,this.limitsInfo=null,this.initialized=!1}};async function u(e,t={}){let{timeoutMs:n=3e4,message:r=`GPU operation timed out`}=t,i,a=new Promise((e,t)=>{i=setTimeout(()=>{t(new o(r))},n)});try{return await Promise.race([e,a])}finally{i!==void 0&&clearTimeout(i)}}function d(e){return e instanceof Error?e.message:String(e)}var f=class e{device;buffers=new Set;constructor(e){this.device=e}static alignSize(e,t){if(t<=0)throw Error(`Alignment must be positive`);return Math.ceil(e/t)*t}createStorageBuffer(t,n){let i=e.alignSize(t.byteLength,4);try{let e=this.device.createBuffer({label:n??`storage-buffer`,size:i,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST,mappedAtCreation:!0});return new Uint32Array(e.getMappedRange()).set(t),e.unmap(),this.buffers.add(e),e}catch(e){throw new r(`Failed to create storage buffer: ${d(e)}`)}}createStagingBuffer(t){let n=e.alignSize(t,4);try{let e=this.device.createBuffer({label:`staging-buffer`,size:n,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});return this.buffers.add(e),e}catch(e){throw new r(`Failed to create staging buffer: ${d(e)}`)}}createUniformBuffer(t,n){let i=e.alignSize(t,16);try{let e=this.device.createBuffer({label:n??`uniform-buffer`,size:i,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return this.buffers.add(e),e}catch(e){throw new r(`Failed to create uniform buffer: ${d(e)}`)}}async readBuffer(t,n){let r=e.alignSize(n,4),a=this.createStagingBuffer(r);try{let e=this.device.createCommandEncoder();e.copyBufferToBuffer(t,0,a,0,r),this.device.queue.submit([e.finish()]),await u(a.mapAsync(GPUMapMode.READ),{message:`Buffer mapping timed out`});let i=a.getMappedRange(),o=new Uint32Array(i.slice(0,n));return a.unmap(),this.releaseBuffer(a),o}catch(e){throw this.releaseBuffer(a),e instanceof o?e:new i(`Failed to read buffer: ${d(e)}`)}}writeBuffer(e,t,n=0){this.device.queue.writeBuffer(e,n,t.buffer,t.byteOffset,t.byteLength)}releaseBuffer(e){this.buffers.has(e)&&(e.destroy(),this.buffers.delete(e))}releaseAll(){for(let e of this.buffers)e.destroy();this.buffers.clear()}getBufferCount(){return this.buffers.size}},p=class{releasers=new Map;track(e,t){return this.releasers.set(e,()=>{if(t){t(e);return}e.destroy()}),e}untrack(e){this.releasers.delete(e)}releaseAll(){for(let e of this.releasers.values())e();this.releasers.clear()}},m=class e{static isSorted(e){if(e.length<=1)return!0;for(let t=0;t<e.length-1;t++)if(e[t]>e[t+1])return!1;return!0}static hasSameElements(e,t){if(e.length!==t.length)return!1;if(e.length===0)return!0;let n=new Map,r=new Map;for(let i=0;i<e.length;i++)n.set(e[i],(n.get(e[i])??0)+1),r.set(t[i],(r.get(t[i])??0)+1);if(n.size!==r.size)return!1;for(let[e,t]of n)if(r.get(e)!==t)return!1;return!0}static validate(t,n){let r=[],i=e.isSorted(n);i||r.push(`Output array is not sorted in ascending order`);let a=e.hasSameElements(t,n);return a||r.push(`Output array does not contain the same elements as input`),{isValid:i&&a,isSorted:i,hasAllElements:a,errors:r}}static compareWithNativeSort(t,n){let r=new Uint32Array(t);r.sort();let i=[],a=!0;if(n.length!==r.length)a=!1,i.push(`Length mismatch: GPU=${n.length}, Native=${r.length}`);else for(let e=0;e<n.length;e++)if(n[e]!==r[e]&&(a=!1,i.push(`Mismatch at index ${e}: GPU=${n[e]}, Native=${r[e]}`),i.length>=5)){i.push(`... (more mismatches not shown)`);break}return{isValid:a,isSorted:e.isSorted(n),hasAllElements:e.hasSameElements(t,n),errors:i}}},h=`// Bitonic Sort WGSL Compute Shader
